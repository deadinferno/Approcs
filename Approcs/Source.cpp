...
LRESULT CALLBACK WndGraphicProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
#pragma region Дескрипторы
HWND hBtnLagr;
HWND hBtnNewt;
HWND hEditMask;
HWND hBackBtn, hGraphBtn, hExampBtn, hExercBtn, hCountBtn, hDrawBtn, hCheckBtn, hGraphicWnd;
HWND hEditDir;
...
bool inWork = false; // Для проверки корректного ввода числа точек многочлена
bool runTable = false;
bool runGraph = false;
bool result = false;
vector<double> X;
...
)
{
	...
	WNDCLASS wc, wcc; 	// Структура для информации о классе окна
	...
	wcc.lpszClassName = ClassName2;		// Имя класса окон
	wcc.lpfnWndProc = (WNDPROC)WndGraphicProc;
	wcc.style = CS_HREDRAW | CS_VREDRAW;	// Стиль класса 
	wcc.hInstance = hInstance;		// Экземпляр приложения
	wcc.hIcon = LoadIcon(NULL, IDI_QUESTION);
	wcc.hCursor = LoadCursor(NULL, IDC_CROSS);
	wcc.hbrBackground = (HBRUSH)GetStockObject(WHITE_BRUSH);
	//wc.hbrBackground = (HBRUSH)GetStockObject(GRAY_BRUSH);
	wcc.lpszMenuName = NULL;		// Ресурс меню окон
	wcc.cbClsExtra = 0;			// Дополнительная память
	wcc.cbWndExtra = 0;			// Дополнительная память
	RegisterClass(&wcc);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		// При нажатии кнопки "Интерполяционный полином Ньютона"
		...
		// При нажатии кнопки "Интерполяционный полином Лагранджа"
		...
		// При нажатии кнопки "Вычислить" при проверке значения полинома в точке
		...
		// При нажатии кнопки "ОК" после ввода количества точек многочлена
		...
		// При нажатии кнопки "Многочлен" после ввода точек многочлена
		case ID_BUTTON_DRAW:
		{
			char out_str[16];
			bool bad = false;
...
LRESULT CALLBACK WndGraphicProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	LPCSTR lpWideStr = "First Windows aplication";
	...
	case WM_PAINT:
	{
		...
		{
			EndPaint(hWnd, &ps);
			MessageBox(hWnd, "Не удалось получить дескриптор экземпляра окна, завершаем приложение.", "Ошибка",
				MB_OK | MB_ICONERROR);
			PostQuitMessage(1);
			return 1L;
		}
		...
		// Если команда нарисовать график - рисовать график
		// Если рисовать = TRUE, то рисуем график
		if (graphicDraw)
		{
			drawCoordinateLines(hDC, hWnd);
			drawGraphic(hDC, hWnd, X, Y, (RGB(0, 0, 255)));
			drawPolynom(hDC, hWnd, X, Y);
		}


		//TextOut(hDC, 350, 350, lpWideStr, strlen(lpWideStr));
...
BOOL drawGraph(HDC hDC, HWND hWnd, int method)
{
	...
	/******************************************************/
	/*		Вывод окна с рисованием графика				  */
	/******************************************************/
	if (graphicDraw)
	{
		hGraphicWnd = CreateWindow(
			ClassName2, 			// Имя класса окон
			"Окно",			// Заголовок окна 
			WS_CHILDWINDOW | WS_VISIBLE | WS_BORDER, 		// Стиль окна
			560,			// X-координаты 
			220,			// Y-координаты 
			400,			// Ширина окна
			400,			// Высота окна
	...
	if (method == LAGR)
	{
		...
		if (result)
		{
			...
			SelectObject(hDC, font25);
			TextOut(hDC, 340, 540, p.c_str(), strlen(p.c_str()));
		}
	}
	else if (method == NEWT)
	{
